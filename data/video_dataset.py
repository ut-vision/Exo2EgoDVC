...
def collate_fn(batch):
    ...
    # feature_list, gt_timestamps_list, labels, caption_list, gt_raw_timestamps, raw_duration, raw_caption, key = zip(*batch)
    feature_list = [_batch['feats'] for _batch in batch]
    feature_size = feature_list[0].shape[1]
    gt_timestamps_list = [_batch['gt_featstamps'] for _batch in batch]
    # track_timestamps_list = [_batch['track_featstamps'] for _batch in batch]
    labels = [_batch['action_labels'] for _batch in batch]
    caption_list = [_batch['caption_label'] for _batch in batch]
    gt_raw_timestamps = [_batch['gt_timestamps'] for _batch in batch]
    # track_raw_timestamps = [_batch['track_timestamps'] for _batch in batch]
    if batch[0]['view_labels'] is not None:
        view_labels = torch.stack([_batch['view_labels'] for _batch in batch])
    else:
        view_labels = None
    raw_duration = [_batch['duration'] for _batch in batch]
    raw_caption = [_batch['captions'] for _batch in batch]
    key = [_batch['key'] for _batch in batch]
    ...
    # track_timestamps = list(chain(*track_timestamps_list))
        ...
        "gt":
            {
                ...
                "timestamp": list(gt_raw_timestamps),  # list (len: video_num) of tensors (shape: (gt_event_num, 2))
                # "track_featstamps": track_timestamps,  # list,        (gt_all_event_num, 2)
                # "track_timestamp": list(track_raw_timestamps),  # list (len: video_num) of tensors (shape: (gt_event_num, 2))
...
class Translator(object):
    def __init__(self, translator_json, vocob_size):
        ...
        print(f"load translator, total_vocab: {len(self.vocab['ix_to_word'])}")
...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_json, is_training, proposal_type, opt):
        ...
        assert os.path.exists(anno_file), f"annot file not found: {anno_file}"
        ...
        print(f'load captioning file from {anno_file}, {len(self.keys)} captioning loaded')
        ...
        if hasattr(self.opt, 'feature_dim_src') and hasattr(self.opt, 'feature_dim_trg'):
            if "egoyc2" in feature_folder[0]:
                self.feature_dim = self.opt.feature_dim_trg
                print("feature dim (egoyc2)", self.feature_dim)
            else:                
                self.feature_dim = self.opt.feature_dim_src
                print("feature dim (yc2)", self.feature_dim)
        else:
            self.feature_dim = self.opt.feature_dim
        self.num_queries = opt.num_queries
        self.tracking_dir = None
        if hasattr(opt, 'tracking_dir'):
            self.tracking_dir = opt.tracking_dir
            if not os.path.exists(self.tracking_dir):
                print(f"tracking dir not found: {self.tracking_dir}")
                self.tracking_dir = None
...
class PropSeqDataset(EDVCdataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_pickle, is_training, proposal_type,
        ...
        super(PropSeqDataset, self).__init__(anno_file,
                                             ...
                                             opt)
        self.data_type = None 
        self.vf_types = vf_types
        print("vf_types: ", self.vf_types)
        if "egoyc2" in anno_file:
            print("loading egoyc2 data")
            self.data_type = "egoyc2"
        elif "yc2" in anno_file:
            print("loading yc2 data")
            self.data_type = "yc2"            
        elif "mmac" in anno_file:
            print("loading mmac data")
            self.data_type = "mmac"
        assert self.data_type is not None, f"unknown data type: {anno_file}"
        
    def load_feats(self, key):
        if self.vf_types is None:
            self.vf_types = self.opt.visual_feature_type
        rescale_method = 'fix_length'
        if type(self.vf_types) == list:
            assert type(self.feature_folder) == list and len(self.vf_types) == len(self.feature_folder), f"{self.vf_types}, {self.feature_folder}"
            feats_dict = {}
            all_padding = True
            for vf_type, vf_folder in zip(self.vf_types, self.feature_folder):
                ...
                if self.opt.data_rescale:
                    if rescale_method == 'fix_length':
                        ...
                        assert follow_type in self.vf_types
            ...
            out = np.concatenate([feats_dict[type_] for type_ in self.vf_types], axis=-1)
        else:
            out, is_padding = get_feats(key, self.vf_types, self.feature_folder, data_norm=self.opt.data_norm)
            if self.opt.data_rescale:
                out = resizeFeature(out, self.opt.frame_embedding_num, 'nearest')
        assert out.shape[1] == self.feature_dim, f"{out.shape[1]}, {self.feature_dim}"
        return out
    
    def load_track_result(self, key):
        if self.tracking_dir is None:
            return None        
        data = read_tracking_file(key, self.tracking_dir)
        return data
    ...
    def __getitem__(self, idx):
        ...
        track_featstamps = None
        if self.data_type == "yc2":
            track_ret = self.load_track_result(key)
            view_labels = torch.zeros(feats.shape[0]).long()
            if track_ret is not None:
                # 0: untracked (ego-like), 1: tracked (exo)
                track_timestamps = np.asarray(track_ret['time_range']).astype(int).tolist()
                track_featstamps = self.process_time_step(duration, track_timestamps, feats.shape[0])                
                for start, end in track_featstamps:
                    view_labels[start:end+1] = 1
        elif self.data_type == "egoyc2":
            # 2: ego
            view_labels = torch.ones(feats.shape[0]).long() * 2            
        else:
            view_labels = torch.zeros(feats.shape[0])
        ...
        sample = {
            ...
            # 'track_featstamps': track_featstamps,
...
def read_tracking_file(video_key, track_folder):
    if video_key.startswith("v_") and len(video_key) > 11:
        video_key = video_key[2:13] #video_key.split("v_", 0)[-1]
    path = os.path.join(track_folder, video_key + '.json')
    if os.path.exists(path):
...
def get_feats(key, vf_type, vf_folder, data_norm=False):
    ...
    if vf_type == 'c3d':
        ...
        if key.startswith("v_") and len(key) > 11:
            path = os.path.join(vf_folder, key[2:13] + '_resnet.npy')
        else:
            path = os.path.join(vf_folder, key + '_resnet.npy')
    elif vf_type == 'resnet_featx4':
        feat_dim = 2048 * 4
        MEAN = 0.41634243404998694
        VAR = 0.2569392081183313
        # NOTE: order: [full, obj_id=1, obj_id=2, obj_id=3]
        if key.startswith("v_") and len(key) > 11:
            path = os.path.join(vf_folder, key[2:13] + '_resnet.npy')
        else:
            path = os.path.join(vf_folder, key + '_resnet.npy')
    elif vf_type == 'resnet-egoyc2':
        feat_dim = 2048
        # MEAN = 0.41634243404998694
        # VAR = 0.2569392081183313
        MEAN = VAR = None
        path = os.path.join(vf_folder, key + '_resnet.npy')
    elif vf_type == 'resnet-mmac':
        feat_dim = 2048
        # MEAN = 0.41634243404998694
        # VAR = 0.2569392081183313
        MEAN = VAR = None
        path = os.path.join(vf_folder, key + '_resnet.npy')
    elif vf_type == 'resnet-egoyc2-mask':
        feat_dim = 8192
        # MEAN = 0.41634243404998694
        # VAR = 0.2569392081183313
        MEAN = VAR = None
        ...
        if key.startswith("v_") and len(key) > 11:
            ...
            path = os.path.join(vf_folder, key + '_bn.npy')            
    ...
    if vf_type == 'resnet_featx4' and feats is not None:
        feats = torch.from_numpy(feats).repeat(1, 4).numpy()
