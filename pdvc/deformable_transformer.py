...
class DeformableTransformer(nn.Module):
    ...
    def set_level_embed(self, hidden_size_list):
        del self.level_embed
        self.level_embed = []
        assert self.num_feature_levels == len(hidden_size_list), f"{self.num_feature_levels}, {len(hidden_size_list)}"
        for hidden_size in hidden_size_list:
            embed_para = nn.Parameter(torch.Tensor(hidden_size))
            normal_(embed_para)
            self.level_embed.append(embed_para)
